describe('Teste de Condução completo do ecommerce de tênis', () => {
    ...
    //     cy.get('tbody > tr').last().find(':nth-child(4) .btn-detalhes').click();
    ...
    //     cy.get('[href="src/vendas/vendasHome.html"]').click()
    //     cy.contains('SELECIONAR USUÁRIO').should('be.visible')
    //     cy.get('#analise-vendas').click()
    //     cy.contains('Análise de Vendas').should('be.visible')
    //     cy.get('#btn-gerar-grafico').click()
    //     cy.get('#grafico-vendas').should('be.visible')
    // })
})